...
public class ExampleAutoPath {
    ...
    private void buildRoutine() {
        ...
        testPath = Builder.holonomicPath(
                ...
                // 4. ADVANCED NODE INTERPOLATOR: Uses nodes to specify the heading of the robot at specific s percentages.
                // NOTE: The C2 Natural Cubic Spline requires a minimum of two nodes to compile successfully!
                .addHeadingNode(0.25, Angle.fromDeg(90))
                .addHeadingNode(0.5, Angle.fromDeg(180))
                .addHeadingNode(0.75, Angle.fromDeg(90))

                // 5. COMPILE: Evaluates geometry and solves a complete optimized kinematically constrained feedforward motion profile.
                // Alternatively, use .quickBuild() for pure geometric lookahead tracking without a physics profile.
                .profiledBuild();
        ...
        testTurn = Builder.turn(testPath.getEndPose())
                ...
                // 7. QUICK BUILD: Locks the turn and finalizes the callback math without running the profile physics.
                // Recommended for stationary turns unless strict dynamic profiling is required via .profiledBuild().
                .quickBuild();
    }
